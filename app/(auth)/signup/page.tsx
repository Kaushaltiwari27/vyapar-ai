    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA] p-4">
      <div className="w-full max-w-md bg-white rounded-[24px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
        ...
        <div className="text-center mb-8">
          <div className="text-sm font-medium text-[#4F46E5] mb-2">Step {step} of 2 — {step === 1 ? 'Create Account' : 'Business Details'}</div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">VyaparAI</h1>
          <p className="text-sm text-gray-500 mt-2">Empower your business with AI</p>
        </div>
      ...
      {step === 1 ? (
        <form onSubmit={handleStep1} className="space-y-4">
          <div><label style={labelStyle}>Full Name</label><input required style={inputStyle} value={formData.fullName} onChange={e => update('fullName', e.target.value)} placeholder="Ramesh Sharma" /></div>
          ...
          <div><label style={labelStyle}>Confirm Password</label><input required type="password" style={inputStyle} value={formData.confirmPassword} onChange={e => update('confirmPassword', e.target.value)} placeholder="Re-enter your password" /></div>
        ...
        <form onSubmit={handleStep2} className="space-y-4">
          <div><label style={labelStyle}>Business Name *</label><input required style={inputStyle} value={formData.businessName} onChange={e => update('businessName', e.target.value)} placeholder="Sharma Textiles" /></div>
          <div><label style={labelStyle}>Owner Name *</label><input required style={inputStyle} value={formData.ownerName} onChange={e => update('ownerName', e.target.value)} placeholder="Ramesh Sharma" /></div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
             <button type="button" onClick={() => setStep(1)} style={{ flex: 1, padding: '12px', borderRadius: 10, border: '1px solid #E5E7EB', background: '#fff', fontSize: 14, cursor: 'pointer', color: '#374151' }}>← Back</button>
        ...
        <p className="mt-8 text-center text-sm text-gray-500">Already have an account? <Link href="/login" className="text-[#4F46E5] font-medium hover:underline">Log in</Link></p>
      </div>
    </div>
  )
}
