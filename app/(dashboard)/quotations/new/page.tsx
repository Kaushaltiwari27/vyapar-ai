...
import { Customer, QuotationItem, Product } from "@/lib/types";
...
export default function NewQuotationPage() {
  ...
  useEffect(() => {
    async function loadInitialData() {
      ...
      let nextNum = "QTN-001";
      if (qtnData && qtnData.length > 0) {
        const lastNum = qtnData[0].quotation_number;
        const parts = lastNum.split('-');
        if (parts.length === 2 && !isNaN(Number(parts[1]))) {
          const num = parseInt(parts[1]) + 1;
          nextNum = `QTN-${num.toString().padStart(3, '0')}`;
        }
      }
  ...
  const handleSave = async (status: 'draft' | 'sent') => {
    ...
    if (error) {
      ...
      toast.success(`Quotation ${status === 'draft' ? 'saved as draft' : 'sent'} successfully`);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Create New Quotation</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-2">
            ...
            <Textarea placeholder="This quotation is valid for 30 days." value={notes} onChange={e => setNotes(e.target.value)} rows={3} />
          </div>
        </div>
